...
void BPF_STRUCT_OPS(kun_running, struct task_struct *p)
{
    ...
    u32 key = 0;
    // Use standard lookup for the current CPU's element
    u64 *cache_misses = bpf_map_lookup_elem(&cache_misses_map, &key);
    u64 *cache_loads = bpf_map_lookup_elem(&cache_loads_map, &key);

    if (cache_misses && cache_loads && *cache_loads > 0) {
        struct task_ctx *tctx = bpf_task_storage_get(&task_ctx_stor, p, 0, 0);
        if (tctx) {
            // Scale by 1000 for precision
            tctx->last_cache_miss_rate = (*cache_misses * 1000) / *cache_loads;
        }
    }
}
...
void BPF_STRUCT_OPS(kun_stopping, struct task_struct *p, bool runnable)
{
    ...
    if (cache_misses && cache_loads && *cache_loads > 0) {
        struct task_ctx *tctx = bpf_task_storage_get(&task_ctx_stor, p, 0, 0);
        if (tctx && tctx->last_cache_miss_rate > 0) {
            u64 current_rate = (*cache_misses * 1000) / *cache_loads;
            
            if (current_rate > tctx->last_cache_miss_rate) {
                u64 diff = current_rate - tctx->last_cache_miss_rate;
                // todo: miss the quant analysis of performance gain from this 
                if ((diff * 100) > (tctx->last_cache_miss_rate * 10)) {
                    tctx->isLimited = true;
                }
            }
        }
