           ...
           u64 wake_flags)
{
    ...
    // Try previous core if different from attached
    ...
    // Try fully idle core in primary if enabled
    if (find_fully_idle) {
    ...
    // Try fully idle core in reserve if enabled
    if (find_fully_idle) {
        cpu = scx_bpf_pick_idle_cpu(cast_mask(p_mask), SCX_PICK_IDLE_CORE);
        if (cpu >= 0) {
    ...
    // Try any idle core in task's mask
